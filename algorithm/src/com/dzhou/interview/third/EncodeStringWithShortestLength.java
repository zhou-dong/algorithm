 ...
 * 
 * Note: k will be a positive integer and encoded string will not be empty or
 * have extra space. You may assume that the input string contains only
 * lowercase English letters. The string's length is at most 160. If an encoding
 * process does not make the string shorter, then do not encode it. If there are
 * several solutions, return any of them is fine.
 * 
 * Example 1:
 * 
 * Input: "aaa" Output: "aaa" Explanation: There is no way to encode it such
 * that it is shorter than the input string, so we do not encode it.
 * 
 * Example 2:
 * 
